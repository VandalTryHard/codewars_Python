...
The rules are simple:
    ...
    if two numbers have the same amount of 1's, the shorter string goes first. (ex: "11" goes before "101" when sorting
    3 and 5 respectively)
    if the strings have the same length, lower decimal number goes first. (ex: 21 = "10101" and 25 = "11001", then 21
    goes first as is lower)
...
def sort_by_binary_ones(numList):
    return sorted(numList, key=lambda k: (-bin(k).count('1'), k))
"""
